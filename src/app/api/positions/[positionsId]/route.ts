...
export async function GET(req: Request, context: { params: Promise<{ positionsId: string }> }) {
  const { positionsId } = await context.params;
  ...
  try {
    ...
    const response = await (
      await fetch(`${process.env.API_URL}/positions/${positionsId}`, {
        headers: {
