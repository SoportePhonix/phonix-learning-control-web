...
export async function GET(_req: Request, { params }: RouteContext) {
  try {
    ...
    const fetchResponse = await fetch(`${process.env.API_URL}/enrollment/student/${studentId}/available-courses`, {
      method: 'GET',
      headers: {
    ...
    if (!fetchResponse.ok) {
      ...
      return errorResponse ?? ApiRes.customError(fetchResponse.status, 'Error fetching available courses');
    }
